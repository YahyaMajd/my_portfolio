...
on a Dockerized NodeJS server. </li>
  ...
  return (
    <section id="projects" className={`projects-section reveal ${inView ? 'visible' : ''}`} ref={ref}>
      <h1>Completed Projects</h1>
      <p> Here are some of the projects I have worked on recently. More information on other projects on my resume!</p>
      <div className={`projects-container reveal-stagger ${inView ? 'visible' : ''}`}>
      ...
      <div className={`projects-container reveal-stagger ${inView ? 'visible' : ''}`}>
        {projectsInProgress.map((project, idx) => (
          <div 
            key={project.id} 
            className="project-card"
            style={{ ['--reveal-delay']: `${idx * 180}ms` }}
